...
public class RegistrationController {
	...
	public Customer registerCustomerForSavingsAccount(@RequestBody Customer customer) throws Exception {
		...
		String message = "Welcome to BitBank.. You have successfully registered with us. \n \n"
				+ "Your application is yet to approve. Kindly wait for atleast 3 working days. \n \n Thank you for choosing us! \n \n  We bank money "
				+ "as well as trust";
		
		service.sendMail(customer.getEmail(), message);
		
		service.sendOTP("  BitBank - Thank you for registering with us."
				+ "Your application is in queue" , mobileNumber);
...
//		}
		...
		String mobileNumber = customerObj.getCustDetails().getMobileNumber();
		service.sendOTP(message, mobileNumber);
		return customerObj;
	}
	@GetMapping(value = "/otp-forget-id-pass/{accNumber}")		// sending OTP to validate for update user request
	...
	public Customer updateUsernamePassword(@PathVariable(value = "accNumber") int accNumber,
		...
		service.updateCustomer(customerObj);
	     System.out.println(customerObj.getPassword());
		return customerObj;
	}
